...
export default function AdminLogs() {
    ...
    if (walletAddress && savedWalletName) {
      ...
      const poller = setInterval(async () => {
        if (window.cardano && window.cardano[savedWalletName.toLowerCase()]) {
          ...
          try {
            const meshWallet = await BrowserWallet.enable(
              savedWalletName.toLowerCase(),
            );
  ...
  const handleConnectClick = async () => {
    try {
      ...
      setErrorModal({
        show: true,
        message:
          "Could not detect any Cardano wallets in this browser." + err.message,
      });
    }
  };
  ...
  const connectSelectedWallet = async (walletName) => {
    try {
      ...
      setErrorModal({
        show: true,
        message: err.message || `Failed to connect ${walletName}.`,
      });
      disconnectWallet();
    }
  };
  ...
  const logToBlockchain = async (log) => {
    if (!walletAddress) {
      setErrorModal({
        show: true,
        message: "Please connect a Cardano wallet first.",
      });
      return;
    }
    ...
    if (walletAddress && !wallet) {
      setErrorModal({
        show: true,
        message:
          "Wallet is still synchronizing. Please wait 2 seconds and try again.",
      });
      return;
    }
    ...
    try {
      ...
      if (!updatedData || updatedData.length === 0) {
        throw new Error(
          "Blockchain Success, but Database Blocked the Save! Please disable Row Level Security (RLS) in Supabase.",
        );
      }
      ...
      if (
        ...
        errorStr.includes("UTxO")
      ) {
        setErrorModal({
          show: true,
          message:
            "Network Busy: Cardano is currently minting your previous transaction. Please wait 20 seconds for the next block before submitting another document!",
        });
      } else {
  ...
  const pushAllPending = async () => {
    const pendingLogs = logs.filter(
      (log) => !log.tx_hash && log.status === "verified",
    );
    ...
    try {
      ...
      setLogs((currentLogs) =>
        currentLogs.map((l) =>
          !l.tx_hash && l.status === "verified"
            ? { ...l, tx_hash: txHash, status: "On-Chain" }
            : l,
        ),
      );
      ...
      setErrorModal({
        show: true,
        message: err.message || "Failed to batch transactions.",
      });
    } finally {
      setIsBatching(false);
    }
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl border border-stone-200 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 relative overflow-hidden">
            ...
            <button
              ...
              className="absolute top-6 right-6 text-stone-400 hover:text-stone-800 transition-colors bg-stone-50 hover:bg-stone-100 p-2 rounded-full"
            >
              <X size={20} className="stroke-[3]" />
            </button>
            <h3 className="font-heading font-extrabold text-2xl text-stone-800 mb-2">
              Connect Wallet
            </h3>
            <p className="text-sm text-stone-500 font-medium leading-relaxed mb-6">
              Choose your preferred Cardano wallet to sign transactions
              securely.
            </p>
            ...
            <div className="flex flex-col gap-3 relative z-10">
              {availableWallets.length === 0 ? (
                ...
                availableWallets.map((w) => (
                  ...
                  >
                    <span className="font-extrabold text-stone-800 capitalize group-hover:text-emerald-700 transition-colors">
                      {w.name}
                    </span>
      ...
      {successModal.show && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl border border-emerald-100 text-center animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 relative overflow-hidden">
            ...
            <p className="text-sm text-stone-500 font-medium leading-relaxed mb-6">
              The document record has been permanently secured on the Cardano
              blockchain.
            </p>
            ...
            <div className="bg-stone-50 p-5 rounded-2xl border border-stone-200 mb-6 overflow-hidden relative z-10 shadow-sm">
              ...
              <p className="font-mono text-xs text-stone-800 truncate">
                {successModal.hash}
              </p>
            </div>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-5xl h-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500 border border-stone-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100 bg-stone-50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-heading font-extrabold text-stone-800 text-lg leading-tight">
                    Document Print Preview
                  </h3>
                  <p className="text-[10px] font-bold text-stone-500 uppercase tracking-widest mt-0.5">
                    {selectedLog.document_type || "Certificate of Indigency"} •{" "}
              ...
              <button
                ...
                className="p-3 bg-white rounded-full text-stone-400 hover:text-red-500 hover:bg-red-50 transition-colors shadow-sm border border-stone-200"
              >
                <X size={20} className="stroke-[3]" />
              </button>
            </div>
            <div className="flex-1 w-full bg-stone-200 p-2 md:p-6">
              <div className="w-full h-full rounded-2xl overflow-hidden shadow-sm border border-stone-300 bg-white">
                <PDFViewer className="w-full h-full border-none">
                  <CertificatePDF
                    docType={selectedLog.document_type}
                    data={selectedLog}
                  />
                </PDFViewer>
              </div>
        ...
        >
          <div>
            ...
            <p className="text-stone-500 text-lg font-medium mt-3 max-w-md">
              Manage, preview, and securely log requested certificates to the
              blockchain.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            {!walletAddress ? (
              <button
                ...
                className="flex items-center gap-2 bg-stone-800 text-white px-6 py-3.5 rounded-xl font-bold hover:bg-black transition-all shadow-lg active:scale-95"
              >
                Connect Wallet
              </button>
            ) : (
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-200 px-5 py-3 rounded-xl font-bold text-sm shadow-sm">
                  ...
                  <span className="capitalize">
                    {localStorage.getItem("cardano_wallet_name")}:
                  </span>
            ...
            <div className="bg-white px-6 py-3 rounded-xl border border-stone-200 shadow-sm flex items-center gap-5">
              <div className="flex flex-col">
                ...
                <span className="text-2xl font-black text-emerald-600 leading-none mt-0.5">
                  {logs.length}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {logs.filter((l) => !l.tx_hash && l.status === "verified")
                  .length > 0 && (
                  <button
                  ...
                  >
                    {isBatching
                      ? "Batching..."
                      : `Push All Verified (${logs.filter((l) => !l.tx_hash && l.status === "verified").length})`}
                  </button>
                )}
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-stone-50/50 border-b-2 border-stone-100">
                  <th className="px-8 py-5 text-[10px] font-black text-stone-400 uppercase tracking-widest whitespace-nowrap">
                    Resident Details
                  </th>
                  <th className="px-8 py-5 text-[10px] font-black text-stone-400 uppercase tracking-widest whitespace-nowrap">
                    Document Type
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-stone-400 uppercase tracking-widest whitespace-nowrap text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-stone-50 text-sm font-semibold text-stone-600">
                {loading ? (
                  <tr>
                    <td
                      colSpan="4"
                      className="px-6 py-20 text-center text-emerald-500"
                    >
                    ...
                    >
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-extrabold text-stone-800 text-base">
                              {log.full_name}
                            </div>
                      ...
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="inline-flex font-extrabold text-sm text-emerald-700">
                            {log.document_type || "Certificate of Indigency"}
                          </span>
                          <span className="text-[10px] text-stone-400 uppercase tracking-wider font-bold mt-1.5">
                            Purpose:{" "}
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex flex-col gap-2">
                          {log.fulfillment_method === "digital" ? (
                            <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-blue-700 bg-blue-100 px-2.5 py-1 rounded-md w-fit border border-blue-200 shadow-sm">
                              Digital
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-orange-700 bg-orange-100 px-2.5 py-1 rounded-md w-fit border border-orange-200 shadow-sm">
                              Physical Pickup
                            </span>
                          )}
                      ...
                      <td className="px-8 py-5 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-3">
                          <button
                            ...
                            className="inline-flex items-center gap-2 text-xs font-bold bg-white text-stone-600 border border-stone-200 px-4 py-2.5 rounded-xl hover:bg-stone-50 hover:text-stone-800 transition-all active:scale-95 shadow-sm"
                          >
                            <Eye size={16} />
                            View
                          </button>
                          ...
                          {log.tx_hash ? (
                            <a
                              href={`https://preview.cardanoscan.io/transaction/${log.tx_hash}`}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-2 text-xs font-bold bg-emerald-500 text-white shadow-md shadow-emerald-500/20 px-4 py-2.5 rounded-xl hover:opacity-90 transition-all active:scale-95"
                            >
                              On-Chain
                            </a>
                          ) : log.status === "pending" ? (
