...
export default function RequestForm() {
  ...
  const validateStrictGmail = (emailString) => {
    ...
    if (!basicRegex.test(lowerEmail))
      return "Please enter a valid email format.";
    if (!lowerEmail.endsWith("@gmail.com"))
      return "To ensure you receive your OTP, this portal strictly requires a valid @gmail.com address.";
    return null;
  };
  ...
  const verifyOTP = async (e) => {
    ...
    try {
      ...
      showModal(
        "Invalid OTP. Please check your email and try again." + err.message,
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const submitRequest = async (e) => {
    ...
    if (!imageFile)
      return showModal(
        "A 2x2 ID picture is strictly required to process your document.",
        "error",
      );
    if (!formData.age || parseInt(formData.age, 10) < 0)
      return showModal("Age must be a valid positive number.", "error");
  ...
  const inputClass =
    "w-full mt-2 bg-stone-50 border border-stone-200 rounded-xl px-4 py-3.5 text-sm text-stone-900 font-medium focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all shadow-sm";
  const labelClass =
    "block text-[10px] sm:text-xs font-bold text-stone-500 uppercase tracking-widest pl-1 leading-snug";
  const isSubmitDisabled =
    !imageFile || loading || !formData.age || parseInt(formData.age, 10) < 0;
      ...
      {modal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl border border-stone-200 text-center animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
            ...
            <h3 className="font-heading font-black text-2xl text-stone-800 mb-2">
              {modal.type === "error" ? "Action Required" : "Notice"}
            </h3>
            ...
            <button
              ...
              className="w-full h-14 inline-flex items-center justify-center rounded-xl bg-stone-800 text-white font-bold transition-all hover:bg-black shadow-lg active:scale-[0.98]"
            >
              Understood
            </button>
          </div>
          ...
          {isCheckingSession ? (
            ...
            <div className="relative z-10">
              {step === 1 && (
                ...
                >
                  <div className="w-16 h-16 bg-emerald-50 flex items-center justify-center rounded-2xl mx-auto text-emerald-600 mb-6 shadow-inner">
                    <span className="text-3xl font-bold">@</span>
                  </div>
                  <h3 className="font-heading font-extrabold text-2xl text-stone-800">
                    Start Request
                  </h3>
                  <p className="text-sm text-stone-500 font-medium">
                    Verify your email address to access the official document
                    request forms.
                  </p>
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full h-14 mt-4 inline-flex items-center justify-center rounded-xl bg-emerald-500 text-white font-bold transition-all hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 disabled:opacity-50 active:scale-[0.98]"
                  >
                ...
                >
                  ...
                  <div className="space-y-4 pt-4 text-left">
                    ...
                    <input
                      ...
                      className={`${inputClass} text-center text-xl tracking-[0.5em] font-black uppercase py-4 bg-stone-100`}
                      value={otp}
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full h-14 inline-flex items-center justify-center rounded-xl bg-stone-800 text-white font-bold transition-all hover:bg-black shadow-lg disabled:opacity-50 active:scale-[0.98]"
                    >
                    ...
                    <div className="flex flex-col gap-3 pt-3">
                      <button
                        ...
                        className="w-full text-xs font-bold text-emerald-600 hover:text-emerald-700 pt-2 uppercase tracking-widest transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        {resendTimer > 0
                ...
                >
                  <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 flex items-center justify-between shadow-sm">
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold uppercase tracking-widest text-stone-500 hover:text-stone-800 transition-colors border border-stone-200 hover:border-stone-300 bg-white px-4 py-2 rounded-xl shadow-sm hover:shadow-md"
                    >
                      Change
                    </button>
                  </div>
                  ...
                  <div className="bg-stone-50/50 border border-stone-200 rounded-[2rem] p-6 shadow-sm">
                    <label
                      className={
                        labelClass + " flex items-center gap-1 text-emerald-700"
                      }
                    >
                    ...
                    <select
                      required
                      className={`${inputClass} appearance-none cursor-pointer text-base font-bold bg-white`}
                    ...
                    >
                      ...
                      <option value="Certificate of Business Operation">
                  ...
                  <div
                    className={`p-6 rounded-[2rem] border transition-colors shadow-sm ${!imageFile ? "bg-stone-50 border-stone-200" : "bg-white border-emerald-100"}`}
                  >
                    <label
                      className={labelClass + " mb-4 flex items-center gap-1"}
                    >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="flex flex-col h-full">
                        {!imagePreview ? (
                          <div className="relative w-full h-48 border-2 border-dashed border-stone-300 rounded-2xl bg-white hover:bg-emerald-50 hover:border-emerald-300 transition-all flex flex-col items-center justify-center cursor-pointer group shadow-sm">
                            ...
                            />
                            <div className="w-14 h-14 bg-stone-50 rounded-full flex items-center justify-center text-stone-400 group-hover:text-emerald-500 group-hover:bg-emerald-100/50 mb-3 group-hover:scale-110 transition-transform">
                            ...
                            <p className="text-xs text-stone-400 font-bold mt-1 uppercase tracking-widest">
                              Required to proceed
                            </p>
                          </div>
                        ) : (
                          <div className="relative w-full h-48 rounded-2xl overflow-hidden border-4 border-white shadow-lg group">
                            ...
                            <div className="absolute inset-0 bg-stone-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                              <button
                                type="button"
                                onClick={removeImage}
                                className="bg-white text-stone-800 p-2.5 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors shadow-lg"
                              >
                      ...
                      <div className="flex flex-col items-center justify-center bg-white rounded-2xl p-5 border border-stone-100 shadow-sm h-48">
                        ...
                        <div className="w-24 h-24 rounded-xl overflow-hidden shadow-sm border border-stone-100 mb-3">
                          <img
                            src={example2x2}
                            alt="Example 2x2"
                        ...
                        <p className="text-xs text-stone-500 text-center font-medium leading-snug px-2">
                          White background, formal attire, clear face without
                          accessories.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      {formData.age && parseInt(formData.age, 10) < 0 && (
                        <p className="text-red-500 text-xs font-bold mt-2 px-1">
                          Invalid Age
                        </p>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    >
                      <option value="digital">
                        Generate Digital PDF (Download)
                      </option>
                      <option value="physical">
                        Request Physical Copy (Onsite Pickup)
                      </option>
                    </select>
                  </div>
              ...
              {step === 4 && (
                ...
                >
                  <div className="w-20 h-20 bg-emerald-500 rounded-3xl flex items-center justify-center mx-auto text-white mb-6 shadow-xl shadow-emerald-500/20">
                    <span className="text-4xl font-bold">&#10003;</span>
                  </div>
                  <div>
                    <h3 className="font-heading font-extrabold text-3xl text-stone-800 mb-2">
                      Success!
                    </h3>
                    <p className="text-sm text-stone-500 font-medium max-w-sm mx-auto">
                      Your request for{" "}
                  ...
                  <div className="bg-stone-50 border border-stone-100 rounded-3xl p-8 mt-8 shadow-sm">
                    {formData.fulfillmentMethod === "digital" ? (
                      <div className="flex flex-col items-center">
                        ...
                        >
                          {({ loading: pdfLoading }) => (
                            <button
                              disabled={pdfLoading}
                              className="group relative inline-flex h-14 w-full sm:w-auto items-center justify-center gap-3 bg-stone-800 text-white px-8 rounded-xl font-bold transition-all hover:bg-black active:scale-[0.98] disabled:opacity-50 shadow-lg"
                            >
                              {pdfLoading
                                ? "Generating..."
                                : "Download PDF Now"}
                      ...
                      <div className="flex flex-col items-center">
                        <h4 className="font-heading font-extrabold text-stone-800 text-xl mb-2">
                          Onsite Pickup
                        </h4>
                        <p className="text-sm text-stone-500 font-medium leading-relaxed max-w-xs">
                          Proceed to Tuyom Barangay Hall during office hours.
                          Present your valid ID upon claiming.
                        </p>
                      </div>
                    )}
                  </div>
