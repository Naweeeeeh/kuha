    ...
    const fetchRequests = async () => {
        try {
            const { data, error } = await supabase
                .from('requests')
                .select('id, created_at, purpose, status')
            ...
            <div className="container max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="text-center mb-12">
                    ...
                    <p className="text-slate-500 text-lg leading-relaxed font-medium max-w-2xl mx-auto">
                        Anonymized public ledger of requested documents, prepared for future Cardano testnet integration.
                    </p>
                </div>
                        ...
                        <div className="overflow-x-auto no-scrollbar">
                            <table className="w-full text-left border-collapse min-w-[600px]">
                                ...
                                <tr className="border-b border-slate-100">
                                    <th className="p-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Tx Hash / ID</th>
                                ...
                                {requests.map((req) => (
                                    <tr key={req.id} className="border-b border-slate-50 hover:bg-[#E4EB9C]/10 transition-colors group">
                                        <td className="p-4 font-mono text-xs text-slate-400 group-hover:text-[#2D5128] transition-colors">{req.id}</td>
                                        <td className="p-4">{new Date(req.created_at).toLocaleDateString()}</td>
                                        <td className="p-4">{req.purpose}</td>
                                        <td className="p-4">
                        <span className={`inline-flex items-center justify-center px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${req.status === 'verified'
                            ? 'bg-[#E4EB9C]/40 text-[#2D5128]'
