...
import { Activity, LinkIcon, Database } from 'lucide-react';
...
export default function Transactions() {
    ...
    return (
        <section className="bg-stone-50 min-h-[calc(100vh-70px)] py-16 px-6 flex flex-col items-center selection:bg-emerald-200 selection:text-emerald-900">
            <div className="container max-w-6xl mx-auto w-full">
                ...
                >
                    ...
                    <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-stone-800 tracking-tight mb-6">
                        Public <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-400">Ledger</span>
                    </h2>
                                    ...
                                    >
                                        <td className="p-5 font-mono text-sm">
                                            {req.tx_hash ? (
                                                ...
                                                >
                                                    {req.tx_hash.substring(0, 16)}...<LinkIcon size={14}/>
                                                </a>
                                            ) : (
                                        ...
                                        <td className="p-5 text-right">
                                            <span className={`inline-flex items-center justify-center px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm transition-all ${
                                                req.status === 'On-Chain' ? 'bg-gradient-to-r from-emerald-500 to-teal-400 text-white shadow-emerald-500/20' :
                                                    req.status === 'verified' ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-500'
                                            }`}>
                                              {req.status}
                                            </span>
                                        </td>
